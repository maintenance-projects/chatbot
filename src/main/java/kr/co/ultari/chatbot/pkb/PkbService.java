...
/**
 * PKB(AI 첨부파일 비서) 도메인: AI 서버 PKB API(명세서 8장, {@code /pkb/{ownerId}/...})로의 릴레이.
 * 개인 지식 저장소로 소유자(ownerId) 단위 관리 → 파티션(dept) 무관하게 dept-less로 호출한다.
 */
@Slf4j
@Service
...
public class PkbService {
    ...
    public SseEmitter ingest(String ownerId, String sender, String roomName,
    ...
    public ResponseEntity<String> files(String ownerId, String category, String tag) {
        ...
        return gateway.get(null, path.toString());
    }

    /** 8.3 파일 상세 정보 (JSON) */
    public ResponseEntity<String> file(String ownerId, String fileHash) {
        return gateway.get(null, "/pkb/" + seg(ownerId) + "/file/" + seg(fileHash));
    }

    /** 8.4 파일 삭제 (JSON) */
    public ResponseEntity<String> delete(String ownerId, String fileHash) {
        return gateway.delete(null, "/pkb/" + seg(ownerId) + "/file/" + seg(fileHash));
    }
