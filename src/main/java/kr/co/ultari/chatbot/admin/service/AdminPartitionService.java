...
/**
 * AI 콜렉션(게이트웨이 용어 partition) 관리 — 게이트웨이 {@code /{dept}/admin/partitions} 프록시.
 * <p>dept(파티션)는 URL <b>prefix</b>로 들어가므로 {@link AiGatewayClient#url(String, String)}의
 * 첫 인자에 dept를 넘긴다(환경설정 {@code /admin/settings/{dept}}가 path에 박는 것과 다름).
 * <ul>
 *   <li>목록 {@code GET  /{dept}/admin/partitions}</li>
 *   <li>생성 {@code POST /{dept}/admin/partitions} — body {@code description=콜렉션이름}(name은 게이트웨이 채번)</li>
...
public class AdminPartitionService {
    ...
    /** 콜렉션 목록 조회 — 게이트웨이 응답을 그대로 통과. */
    public ResponseEntity<String> list(String dept) {
        return gateway.get(dept, "/admin/partitions");
    }

    /** 콜렉션 생성 — description(사용자 입력 이름)만 multipart로 전달, name은 게이트웨이가 채번. */
    ...
    /**
     * 콜렉션 이름 변경 — 식별자(name)는 경로로 유지, description(표시명)만 새 값으로 갱신.
    ...
    /**
     * 콜렉션 삭제 — 식별자(name)를 경로에 넣어 전달(URL 인코딩).
     * 게이트웨이 삭제 성공(code 0000) 시에만 해당 콜렉션의 접근 권한(AI_COLLECTION_GRANT) orphan을 함께 정리한다.
     */
    public ResponseEntity<String> delete(String dept, String name) {
        ...
        if (res.getStatusCode().is2xxSuccessful() && isOk(res.getBody())) {
            List<AiCollectionGrant> grants =
                    collectionGrantRepository.findByAiDeptAndCollectionName(dept, name);
            if (!grants.isEmpty()) {
                collectionGrantRepository.deleteAll(grants);
